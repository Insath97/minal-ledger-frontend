        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-5">
            {buckets.map((b) => {
              ...
              return (
                <div key={b.key} className={`rounded-2xl border ${style.border} ${style.bg} p-3 sm:p-5 shadow-sm`}>
                  <p className="text-[11px] sm:text-xs text-slate-500">{b.label}</p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-3 sm:p-5 shadow-sm">
              <p className="text-[11px] sm:text-xs text-slate-500">Total Due</p>
              <p className="text-base sm:text-lg font-bold text-slate-900">{formatCurrency(data.summary.total_due)}</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            ...
            <div className="md:hidden divide-y divide-slate-50">
              {data.sales.length === 0 ? (
                <div className="px-4 py-16 text-center"><Clock className="mx-auto h-10 w-10 text-slate-300 mb-3" /><p className="text-sm font-semibold text-slate-600">No outstanding dues</p></div>
              ) : (
                data.sales.map((s) => {
                  const style = BUCKET_STYLES[s.aging_bucket] || BUCKET_STYLES["0-30 days"];
                  return (
                    <div key={s.sale_id} className="px-4 py-3 space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-slate-500">{Math.floor(s.days_outstanding)} days</p>
                        </div>
                      </div>
