...
<td width="50%" style="text-align:right;font-size:9px;color:#94a3b8">Generated by: System<br>${nowFull}</td>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-foreground">Customer Statement</h1>
        <p className="mt-1 text-xs sm:text-sm text-muted-foreground">Full transaction history for a customer.</p>
      </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1 min-w-0" ref={customerDropdownRef}>
            <button type="button" onClick={() => setCustomerDropdownOpen(!customerDropdownOpen)} className="flex h-11 w-full items-center justify-between gap-2 rounded-lg border border-border bg-card px-3 text-sm transition-all hover:border-border focus:border-emerald-500">
              <span className={`truncate ${selectedCustomer ? "text-foreground font-medium" : "text-muted-foreground"}`}>{selectedCustomer ? `${selectedCustomer.name} (${selectedCustomer.code})` : "Select a customer"}</span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {hasFilters && <button onClick={clearFilters} className="h-11 px-4 rounded-lg bg-red-500/10 text-xs font-medium text-red-500 hover:bg-red-500/10 transition-colors whitespace-nowrap">Clear</button>}
      ...
      {loading ? (
        ...
        <>
          <div className="rounded-2xl border border-border bg-card p-4 sm:p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
              <div><h3 className="text-base font-bold text-foreground">{data.customer.name}</h3><p className="text-sm text-muted-foreground">{data.customer.code} &middot; {data.customer.phone}</p></div>
              <div className="sm:text-right"><p className="text-xs text-muted-foreground">Outstanding Balance</p><p className="text-lg font-bold text-red-600">{formatCurrency(data.customer.outstanding_balance)}</p></div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="rounded-xl bg-emerald-500/10 p-4"><p className="text-xs text-emerald-600">Total Sales</p><p className="text-lg font-bold text-emerald-600">{formatCurrency(data.summary.total_sales)}</p></div>
              <div className="rounded-xl bg-blue-500/10 p-4"><p className="text-xs text-blue-600">Total Payments</p><p className="text-lg font-bold text-blue-600">{formatCurrency(data.summary.total_payments)}</p></div>
              <div className="rounded-xl bg-muted p-4"><p className="text-xs text-muted-foreground">Net Balance</p><p className="text-lg font-bold text-foreground">{formatCurrency(data.summary.net_balance)}</p></div>
            </div>
          </div>

          <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
            <div className="px-4 sm:px-6 py-4 border-b border-border"><h3 className="text-sm font-semibold text-foreground">Transactions</h3></div>
            <div className="hidden md:block overflow-x-auto scrollbar-thin">
              <table className="w-full min-w-[640px]">
                <thead><tr className="border-b border-border bg-muted/50">
                  {["Date","Type","Description","Ref","Debit","Credit","Balance"].map((h) => <th key={h} className={`px-4 sm:px-5 py-3 text-[11px] font-bold uppercase tracking-wider text-muted-foreground ${["Debit","Credit","Balance"].includes(h) ? "text-right" : "text-left"}`}>{h}</th>)}
                </tr></thead>
                <tbody className="divide-y divide-border">
                  {data.transactions.length === 0 ? (
                    <tr><td colSpan={7} className="px-5 py-16 text-center"><UserCheck className="mx-auto h-10 w-10 text-muted-foreground/50 mb-3" /><p className="text-sm font-semibold text-foreground">No transactions found</p></td></tr>
                  ) : data.transactions.map((txn, i) => (
                    <tr key={i} className="hover:bg-accent/50 transition-colors">
                      <td className="px-4 sm:px-5 py-3 text-sm text-foreground whitespace-nowrap">{new Date(txn.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</td>
                      <td className="px-4 sm:px-5 py-3"><span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${txn.type === "sale" ? "bg-blue-500/10 text-blue-600 border border-blue-500/20" : "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20"}`}>{txn.type === "sale" ? "Sale" : "Payment"}</span></td>
                      <td className="px-4 sm:px-5 py-3 text-sm text-foreground max-w-[200px] truncate">{txn.description}</td>
                      <td className="px-4 sm:px-5 py-3 text-sm font-mono text-foreground">{txn.reference}</td>
                      <td className="px-4 sm:px-5 py-3 text-right text-sm font-semibold text-foreground">{txn.debit > 0 ? formatCurrency(txn.debit) : "\u2014"}</td>
                      ...
                      <td className="px-4 sm:px-5 py-3 text-right text-sm font-bold text-foreground">{formatCurrency(txn.balance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="md:hidden divide-y divide-border">
              {data.transactions.length === 0 ? (
                <div className="px-4 py-16 text-center"><UserCheck className="mx-auto h-10 w-10 text-muted-foreground/50 mb-3" /><p className="text-sm font-semibold text-foreground">No transactions found</p></div>
              ) : data.transactions.map((txn, i) => (
                <div key={i} className="px-4 py-3 space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${txn.type === "sale" ? "bg-blue-500/10 text-blue-600 border border-blue-500/20" : "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20"}`}>{txn.type === "sale" ? "Sale" : "Payment"}</span>
                    <span className="text-xs text-muted-foreground">{new Date(txn.date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}</span>
                  </div>
                  <p className="text-sm text-foreground truncate">{txn.description}</p>
                  <div className="flex items-center justify-between text-sm">
                    <span className="font-mono text-muted-foreground text-xs">{txn.reference}</span>
                    <div className="flex gap-3">
                      {txn.debit > 0 && <span className="font-semibold text-foreground">Dr {formatCurrency(txn.debit)}</span>}
                      {txn.credit > 0 && <span className="font-semibold text-emerald-600">Cr {formatCurrency(txn.credit)}</span>}
                    </div>
                  </div>
                  <p className="text-right text-sm font-bold text-foreground">Bal: {formatCurrency(txn.balance)}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-dashed border-border bg-card p-12 sm:p-16 text-center">
          ...
          <p className="text-sm font-semibold text-foreground">Select a customer to view their statement</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-2xl bg-card p-4 sm:p-6 shadow-2xl">
            <div className="mb-3 sm:mb-5 text-center">
              ...
              <h3 className="text-sm sm:text-base font-bold text-foreground">Print Statement</h3>
